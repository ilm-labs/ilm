...
vr = Ilm::Chemical::Reactors::Reactor.new(input: { substance: s, quantity: '10 kg' },
...
RSpec.describe 'Ilm::Chemical::Reactors::Reactor#new' do
  ...
  %i[input output].each do |feed|
    it "should initialize with correct #{feed}" do
      ...
      expect { Ilm::Chemical::Reactors::Reactor.new(feed => { substance: 'c' }).send(feed) }
        .to raise_error(ArgumentError, 'Invalid substance')
      expect { Ilm::Chemical::Reactors::Reactor.new(feed => { substance: s }).send(feed) }
        .to raise_error(ArgumentError, 'Invalid quantity')
      expect { Ilm::Chemical::Reactors::Reactor.new(feed => { quantity: '10 kg' }).send(feed) }
        .to raise_error(ArgumentError, 'Invalid substance')
      expect { Ilm::Chemical::Reactors::Reactor.new(feed => { substance: s, quantity: '10' }).send(feed) }
        .to raise_error(ArgumentError, 'Invalid quantity')
      expect { Ilm::Chemical::Reactors::Reactor.new(feed => { substance: s, quantity: '10 kl' }).send(feed) }
